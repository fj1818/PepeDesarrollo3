...
function initComunicaciones() {
    try {
        ...
        // Asegurar que la función es global
...
// AÑADIDO PARA ASEGURAR DISPONIBILIDAD GLOBAL DE FUNCIONES
// Exportar las funciones principales al objeto window
(function exportarFuncionesGlobales() {
    console.log('Exportando funciones principales al objeto global window');
    
    // Funciones principales a exportar
    const funcionesExportar = [
        'initComunicaciones',
        'loadComunicacionesUI',
        'loadComunicacionesContent',
        'loadContacts',
    ...
    // Exportar cada función si está disponible
    funcionesExportar.forEach(nombreFuncion => {
        if (typeof window[nombreFuncion] === 'undefined' && typeof eval(nombreFuncion) === 'function') {
            console.log(`Exportando función ${nombreFuncion} al objeto window`);
            window[nombreFuncion] = eval(nombreFuncion);
        }
    });
    
    // Si initComunicaciones no está definida en este contexto, usar loadComunicacionesUI directamente
    if (typeof window.initComunicaciones === 'undefined') {
        console.warn('initComunicaciones no está disponible, definiendo una función alternativa');
        window.initComunicaciones = function() {
            console.log('Función alternativa de initComunicaciones llamada');
            if (typeof window.loadComunicacionesUI === 'function') {
                window.loadComunicacionesUI();
            } else if (typeof loadComunicacionesUI === 'function') {
                loadComunicacionesUI();
            } else {
                console.error('No se pudo cargar la UI de comunicaciones');
            }
        };
    }
    
    console.log('Exportación de funciones completada');
})();